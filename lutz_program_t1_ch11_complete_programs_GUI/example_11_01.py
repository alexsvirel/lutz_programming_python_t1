"""
Модуль с начальными настройками PyEdit (textConfig.py)

Общие настройки
- закомментируйте любые настройки в этом разделе, чтобы принять настройки по
умолчанию библиотеки Tk или программы;
- шрифт/цвет можно также менять из меню в графическом интерфейсе, а также менять
размеры окон после их открытия;
- импортируются из пути поиска модулей:
-- могут определять отдельные настройки для каждого клиентского приложения,
