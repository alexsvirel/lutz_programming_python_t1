"""
PyEdit 2.1: Текстовый редактор и компонент на Python/tkinter (textEditor.py).
...
-supports arbitrary Unicode encodings for files: per example_11_01.py settings
...
from examples.guimaker import *  # Frame + menu/toolbar builders

# general configurations
try:
    import example_11_01  # startup font and colors

    configs = example_11_01.__dict__  # work if not on the path or bad
...
- startup configuration module example_11_01.py
"""
...
class TextEditor:  # mix with menu/toolbar Frame class
    ...
    if __name__ == '__main__':
        from example_11_01 import (  # my dir is on the path
            opensAskUser, opensEncoding,
            savesUseKnownEncoding, savesAskUser, savesEncoding)
    else:
        from .example_11_01 import (  # 2.1: always from this package
            opensAskUser, opensEncoding,
            savesUseKnownEncoding, savesAskUser, savesEncoding)
    ...
    def onOpen(self, loadFirst='', loadEncode=''):
        ...
        an encoding passed in, input from the user, in example_11_01, or
        ...
        encoding behavior is configurable in the local example_11_01.py:
