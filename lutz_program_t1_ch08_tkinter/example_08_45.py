"""
выводит все изображения, в каталоге, в виде миниатюр на кнопках

щелчок на миниатюрах (на самом деле на кнопках) приводит к выводу полноразмерного изображения;
требуется наличия пакета PIL для отображения JPEG-файлов и создания миниатюр;

если в окне выводится слишком много миниатюр, добавляется прокрутка
"""
import math
import os
import sys
    ...
    for imgfile in os.listdir(imgdir):
        ...
        if os.path.exists(thumbpath):
            ...
            try:
                ...
                imgobj.thumbnail(size, Image.ANTIALIAS)  # фильтр, дающий лучшее качество при
                # уменьшении размеров
...
def viewer(imgdir, kind=Toplevel, cols=None):
    """
    создает окно с миниатюрами для каталога с изображениями: по одной кнопке сминиатюрой для каждого изображения;
    используйте параметр kind=Tk, чтобы вывести миниатюры в главном окне, или Frame (чтобы прикрепить к фрейму);
    значение imgfile изменяется в каждой итерации цикла: ссылка на значение должна сохраняться по умолчанию;
    ...
    quit = Button(win, text='Quit', command=win.quit, bg='beige')  # добавить первой, чтобы урезалась последней
