...
def makemenu(win):
    ...
    edit = Menu(top, tearoff=False) # tearoff - создание "отрывающегося" меню (здесь запрещено)
    edit.add_command(label='Cut', command=notdone, underline=0)
    edit.add_command(label='Paste', command=notdone, underline=0)
    edit.add_separator()            # создать визуальный разделитель пунктов меню
    top.add_cascade(label='Edit', menu=edit, underline=0)

    submenu = Menu(edit, tearoff=True)  # tearoff - создание "отрывающегося" меню разрешено
