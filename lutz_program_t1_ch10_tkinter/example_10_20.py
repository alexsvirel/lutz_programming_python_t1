...
# будет извлечен из очереди и выполнен функцией threadChecker в главном потоке;
#################################################################################
...
if __name__ == '__main__':  # самотестирование при запуске в виде сценария
    ...
    def onEvent(i):  # code that spawns thread реализация порождения потоков
        myname = 'thread-%s' % i
        startThread(
            action=threadaction,
            args=(i, 3),
    ...
    # thread's main action основная операция, выполняемая потоком
    def threadaction(id, reps, progress):  # what the thread does то, что делает поток
        for i in range(reps):
            time.sleep(1)
            if progress: progress(i)  # progress callback: queued
            # обработчик progress: в очередь
        if id % 2 == 1: raise Exception  # odd numbered: fail
        # ошибочный номер: неудача
